...
docker run --network host --env TEST_SHARD=$TEST_SHARD --env RUN_TEST="buildfarm/.bazelci/run_generative_cc_many_test.sh" buildfarm buildfarm/.bazelci/test_buildfarm_container.sh