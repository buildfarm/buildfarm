...
public abstract class AbstractServerInstance implements Instance {
  ...
  public String getBlobName(Digest blobDigest) {
    return String.format("%s/blobs/%s", getName(), DigestUtil.toString(blobDigest));
  }
  ...
  public ListenableFuture<Void> execute(
    ...
    if (!skipCacheLookup) {
      metadata = metadata.toBuilder().setStage(ExecutionStage.Value.CACHE_CHECK).build();
    ...
    if (actionResult != null) {
      ...
      metadata = metadata.toBuilder().setStage(ExecutionStage.Value.QUEUED).build();
    }

    operation = operationBuilder.setMetadata(Any.pack(metadata)).build();
        ...
        new FutureCallback<ByteString>() {
          ...
          public void onSuccess(ByteString blob) {
            try {
              ...
              logger.log(
                  WARNING, format("expect parse for %s failed", DigestUtil.toString(digest)), e);
  ...
  public boolean requeueOperation(Operation operation) throws InterruptedException {
    ...
    if (metadata == null) {
      ...
      String message =
          String.format("Operation %s does not contain ExecuteOperationMetadata", name);
      errorOperation(
          operation,
          requestMetadata,
          com.google.rpc.Status.newBuilder()
    ...
    if (metadata.getStage() != ExecutionStage.Value.QUEUED) {
      // ensure that watchers are notified
      String message = String.format("Operation %s stage is not QUEUED", name);
      errorOperation(
          operation,
          requestMetadata,
          com.google.rpc.Status.newBuilder()
