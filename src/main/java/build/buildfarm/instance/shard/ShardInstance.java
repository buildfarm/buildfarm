...
public class ShardInstance extends AbstractServerInstance {
                  ...
                  poller.resume(
                      () -> {
                        try {
                          ...
                          if (!stopping && !stopped) {
                            logger.log(
                                SEVERE, format("error polling %s for queuing", operationName), e);
                          }
                            ...
                            .addListener(
                                () -> {
                                  try {
                                    ...
                                    logger.log(
                                        SEVERE, "interrupted while returning transform token", e);
                                  }
                                },
                                operationTransformService);
                      } catch (IOException e) {
  ...
  String requestFormat(RequestMetadata request, String f, Object... args) {
    return format(
            "%s -> %s -> %s: ",
            request.getCorrelatedInvocationsId(),
            request.getToolInvocationId(),
            request.getActionId())
        + format(f, args);
  }
  ...
  abstract static class TreeCallback implements FutureCallback<Directory> {
    ...
    protected abstract void onDirectory(Directory directory);
  ...
  private void removeMalfunctioningWorker(String worker, Throwable t, String context) {
    try {
      if (backplane.removeWorker(worker, format("%s: %s", context, t.getMessage()))) {
        logger.log(
            WARNING, format("Removed worker '%s' during(%s) due to exception", worker, context), t);
      }
    } catch (IOException e) {
      throw Status.fromThrowable(e).asRuntimeException();
    }
  ...
  private boolean checkCache(
      ActionKey actionKey, Operation operation, RequestMetadata requestMetadata) throws Exception {
    ExecuteOperationMetadata metadata =
        ExecuteOperationMetadata.newBuilder()
  ...
  private ListenableFuture<Boolean> checkCacheFuture(
    ...
    ListenableFuture<Boolean> checkCacheFuture =
        operationTransformService.submit(
