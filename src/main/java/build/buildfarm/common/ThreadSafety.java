 ...
 * mainly just to standardize the terminology. (However, if this catches on, I can also imagine in
 * the future having a presubmit check that checks that all new classes have thread safety
 * annotations :)
 *
...
public class ThreadSafety {
   ...
   *
   * <p>When applied to a class, this attribute indicates that instances of the class can safely be
   * used concurrently from multiple threads without any need for external synchronization, i.e.
   * that all non-static methods are thread-safe (except any private methods that are explicitly
   * annotated with a different thread safety annotation). In addition it also indicates that all
   * non-static nested classes are thread-safe (except any private nested classes that are
   * explicitly annotated with a different thread safety annotation). Note that no guarantees are
   * made about static class methods or static nested classes - they should be annotated separately.
   *
   * <p>When applied to a method, this attribute indicates that the method can safely be called
   * concurrently from multiple threads. The implementation must synchronize any accesses to mutable
   * data.
   */
  @Documented
   ...
   *
   * <p>The implementation of such a class must synchronize accesses to mutable static data, but can
   * assume that each instance will only be accessed from one thread at a time.
   *
   * <p>The client must obtain an appropriate lock before calling ThreadCompatible methods, or must
   * otherwise ensure that only one thread calls such methods. Unless otherwise specified, an
   * appropriate lock means synchronizing on the instance.
   *
   * <p>A ThreadCompatible class may contain private methods or private nested classes that are not
   * ThreadCompatible provided that they are explicitly annotated with a different thread safety
   * annotation.
   */
  @Documented
   ...
   *
   * <p>The methods (and nested classes) of a ConditionallyThreadSafe class should each have their
   * thread safety marked.
   */
  @Documented
  @Target(value = {ElementType.METHOD, ElementType.TYPE})
  @Retention(RetentionPolicy.SOURCE)
   ...
   *
   * <p>The methods (and nested classes) of a ConditionallyThreadCompatible class should each have
   * their thread safety marked.
   */
  @Documented
  @Target(value = {ElementType.METHOD, ElementType.TYPE})
  @Retention(RetentionPolicy.SOURCE)
