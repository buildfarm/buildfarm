...
import build.buildfarm.common.cache.CacheBuilder.OneWeigher;
...
class LocalCache<K, V> extends AbstractMap<K, V> implements ConcurrentMap<K, V> {
  ...
  static class Segment<K, V> extends ReentrantLock {
    ...
    ListenableFuture<V> get(K key, int hash, CacheLoader<? super K, V> loader) {
      checkNotNull(key);
      checkNotNull(loader);
      ...
      if (count != 0) { // read-volatile
        ...
        if (e != null) {
          ...
          if (value != null) {
            ...
            future = scheduleRefresh(e, key, hash, value, now, loader);
          } else {
            ValueReference<K, V> valueReference = e.getValueReference();
            if (valueReference.isLoading()) {
              future = waitForLoadingValue(e, key, valueReference);
            } else {
              future = lockedGetOrLoad(key, hash, loader);
            }
          }
        } else {
          future = lockedGetOrLoad(key, hash, loader);
      ...
      future.addListener(this::postReadCleanup, directExecutor());
      return future;
    }
    ...
    // at most one of loadAsync/loadRefreshAsync may be called for any given LoadingValueReference
    ...
    ListenableFuture<V> loadRefreshAsync(
        final K key,
        final int hash,
    ...
    ListenableFuture<V> getAndRecordStats(
        K key,
        int hash,
        LoadingValueReference<K, V> loadingValueReference,
    ...
    ListenableFuture<V> scheduleRefresh(
      ...
      if (map.refreshes()
        ...
        ListenableFuture<V> newValue = refresh(key, hash, loader, true);
        if (newValue != null) {
          return newValue;
        }
      }
    ...
    ListenableFuture<V> refresh(K key, int hash, CacheLoader<? super K, V> loader, boolean checkTime) {
      ...
      return loadRefreshAsync(key, hash, loadingValueReference, loader);
    }
  ...
  ListenableFuture<V> get(K key, CacheLoader<? super K, V> loader) {
  ...
  ListenableFuture<V> getOrLoad(K key) {
      ...
      if (!keysToLoad.isEmpty()) {
        try {
          ...
          for (K key : keysToLoad) {
            ...
            result.put(key, getUninterruptibly(get(key, defaultLoader)));
          }
        }
      }
      return ImmutableMap.copyOf(result);
  ...
  static class LocalLoadingCache<K, V> extends LocalManualCache<K, V>
    ...
    @Override
    public V getUnchecked(K key) {
      // same as AbstractLoadingCache...
      try {
        return getUninterruptibly(get(key));
      } catch (ExecutionException e) {
