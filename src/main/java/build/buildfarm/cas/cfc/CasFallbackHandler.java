 ...
 *  @brief   The CasFileCache provides a fallback mechanism to store CAS data
 *           in another data source. This module handles the various fallback
 *           behaviors.
...
public class CasFallbackHandler {
     ...
     */
static   public  void start(ContentAddressableStorage delegate, Consumer<Digest> onStartPut, ExecutorService removeDirectoryService, boolean skipLoad) throws IOException,InterruptedException
{
...
// start delegate if we specifically have a CASFileCache
     ...
     */
static   public  InputStream newTransparentInput(ContentAddressableStorage delegate, NoSuchFileException e, Digest digest, long offset) throws IOException
{
    if (delegate == null) {
      throw e;
    }
     ...
     */
static   public  Iterable<Digest> findMissingBlobs(ContentAddressableStorage delegate, ImmutableList<Digest> missingDigests) throws InterruptedException
{
