...
| processorCountSource              | JAVA_RUNTIME/OSHI/MAX_OF_SOURCES, _MAX_OF_SOURCES_ |     | Source for the processor count used to auto-size `executeStageWidth` (when 0) and the cache thread pools. `JAVA_RUNTIME` honors the container's cgroup CPU limit; `OSHI` reports host processors; `MAX_OF_SOURCES` takes the larger of the two (legacy default, guards against under-reporting in some containers)          |
