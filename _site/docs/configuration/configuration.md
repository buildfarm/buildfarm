...
| streamQueue                        | boolean, _false_                         |                 |                       | Use Redis Streams with consumer groups instead of Redis Lists for operation queues. Provides built-in pending entry tracking and atomic acknowledgment. See [Redis Stream Queues](../architecture/redis-stream-queues). Takes precedence over `priorityQueue` if both are set. |
| streamPelMonitorIntervalSeconds    | Integer, _30_                            |                 |                       | How often (in seconds) the StreamPelMonitor checks for stale Pending Entries List entries. Only used when `streamQueue` is `true`. |
| streamPelMinIdleMillis             | Long, _60000_                            |                 |                       | Minimum idle time (in milliseconds) before a pending stream entry is reclaimed via XAUTOCLAIM. Only used when `streamQueue` is `true`. |
...
| zstdBufferPoolBorrowTimeoutMillis | Integer, _-1_                 |                       | Specifies how long a zstd transfer waits for a free data buffer. A negative value waits without a limit. A value of `0` fails a transfer that cannot take a free buffer at once.                                                                                                                                         |
| compressedBlobTransfer            | boolean, _false_              | COMPRESSED_BLOB_TRANSFER | Use zstd compression when transferring blobs to/from remote storages |
